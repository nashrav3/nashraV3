...
feature.command(
  ...
  async (ctx) => {
    ...
    const matchResult = ctx.message.text.match(
      /\s+(?<postNumber>\d+)(?=(?:.*\bat\s+(?<sendAt>\d+(?::\d+)?\s*\S+))?)(?=(?:.*\bfor\s+(?<duration>\d+\s*\S+))?)(?=(?:.*\buntil\s+(?<until>\d+(?::\d+)?\s*\S+))?)/i
    );
    if (matchResult === null) return; // TODO: reply with error message

    const postNumber = matchResult.groups?.postNumber;
    const sendAt = matchResult.groups?.sendAt;
    const duration = matchResult.groups?.duration;
    const until = matchResult.groups?.until;

    await ctx.reply(
      `post ${postNumber}, sendAt ${sendAt}, duration ${duration}, until ${until}`
    );
